 ...
 * @Before 里面的是切入点表达式: 
 * 
 * 6. 在通知中访问连接细节: 可以在通知方法中添加 JoinPoint 类型的参数, 从中可以访问到方法的名称和方法的参数. 
 * 
 * 7. @After 表示后置通知: 在方法执行之后执行的代码. 
 */

//通过添加 @Aspect 注解声明一个 bean 是一个切面!
//把这个类声明为一个切面：先把该类放入到IOC容器中（@Component），再声明为一个切面（@Aspect）
/**
...
public class LoggingAspect {
	 ...
	 */
	@Pointcut("execution(public int com.cxq.spring.aop.aspectJ.ArithmeticCalculator.*(..))")
	public void declareJointPointExpression(){}

	@Before("execution(public int com.cxq.spring.aop.aspectJ.ArithmeticCalculator.*(int, int))")
	public void beforeMethod1(){
		System.out.println("The method begins");
	}
	
	//前置通知：声明该方法事一个前置通知，在目标方法开始前执行
	@Before("execution(public int com.cxq.spring.aop.aspectJ.ArithmeticCalculator.*(int, int))")
	...
	//后置通知中，不能访问目标方法执行的结果，需要在返回通知中去访问
